var RadarChart = {
  defaultConfig: {
    ...
    w: 340,
    h: 340,
    factor: 0.6, // Este factor es el tamaño del grafico
    factorLegend: 0.9, // Este factor es la amplitud de las letras , para que se vea aceptable, etre los dos factores debe existir almenos 0.2 de gap
    levels: 10,
    levelTick: false,
	TickLength: 10,
    maxValue: 10,
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        if(cfg.axisLine || cfg.axisText) {
          ...
          for (var i = 0 ; i <= allAxis.length -1; i++) {
            var splitName = allAxis[i]['name'];
            splitName = splitName.split("<br />");
		  ...
		  //Se agrega la linea que no se estaba agregando
		  	axis.append("line")
		.attr("x1", cfg.w/2)
		.attr("y1", cfg.h/2)
